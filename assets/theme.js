...
function ttProductHover() {
  $document.on('click', '.tt-img-parent', function(e){
    e.preventDefault();
  })

      /*
    let temp = e.target.parentElement.parentElement.href + ".js";
    var temp_data;
    console.log(temp_data);
    console.log(container.getElementsByTagName("UL")[1]);
    $(container.getElementsByTagName("UL")[1]).on('click', function(variant_select){
      console.log(variant_select.target.dataset.tooltip);
    })
    fetch(temp).then(res => {return res.json()})
      .then(data => console.log(data))
      .then(variant => console.log(variant));
    
    */

  $document.on('mouseenter mouseleave', '.tt-img', function(e) {

    let container = e.currentTarget.parentElement.parentElement.parentElement;
    let img_ele = container.querySelector(".tt-img").firstElementChild ;

    if (e.type === 'mouseenter' && window.innerWidth > 1024) {
      img_ele.src = container.dataset.lifestyle;
      console.log(img_ele)
      console.log(img_ele.src)

    } else if (e.type === 'mouseleave' && window.innerWidth > 1024) {
      img_ele.src = `${container.dataset.main}`;
    };
  })
...
function ttCollapse() {
  ...
  itemTitle.on('click', function(e) {
    ...
    var thisParent = $(this).parent(),
      ...
      let height = nextLevel.height() + 30;
     console.log(height);
    if (thisParent.hasClass('open')) {
      $('.tt-collapse').not($(this).parent()).removeClass('open');
      // $('.tt-collapse-content').slideUp();
      $('.tt-collapse-content').hide();
      thisParent.removeClass('open');
      // nextLevel.slideUp(speed);
      nextLevel.hide();
      if($(this).text() == "Collection"){$('.collection_filter_bottom_column').removeClass('cfilteropen');$('.collection_filter_bottom_column').css('margin-top',10+"px");}
      else{$('.collection_filter_bottom_column').removeClass('filteropen');}
      ...
      if($(this).text() == "Collection"){
        $('.collection_filter_bottom_column').css('margin-top',height+"px");
        $('.collection_filter_bottom_column').addClass('cfilteropen');
        $('.collection_filter_bottom_column').removeClass('filteropen');
      }
      else{
         $('.collection_filter_bottom_column').css('margin-top',height+"px");
        $('.collection_filter_bottom_column').addClass('filteropen');
        $('.collection_filter_bottom_column').removeClass('cfilteropen');}
       // $('.collection_filter_bottom_column').addClass('filteropen');
    }
  })
};
...
function ttFilterLayout(ttwindowWidth){
  ...
  if($ttFilterOptions.hasClass('desctop-no-sidebar') && !$ttFilterOptions.hasClass('filters-detach-mobile')){
     ttwindowWidth <= 767 ?  insertMobileCol() : insertFilter();
  };
  ...
  function ttСhangeclass(ttObj, ttObjvalue){
    ttwindowWidth <= 767 ?  ttShowIconMobile(ttObj, ttObjvalue) : ttShowIconDesctop(ttObj, ttObjvalue);
...
function setNewData($this, combobox){
  ...
  $pr_parent.find('input[name="id"]').val(_parent.attr('data-var_id'));
  ...
  $btn.attr("href", "javascript:addItemToCart('" + _parent.attr('data-var_id') + "', 1)");
  $btn.attr("onclick", "addItemToCart('" + _parent.attr('data-var_id') + "', 1)");
...
$.fn.initRevolution = function(){
  ...
  var delay = this.data('speed'),
      fullscreen = this.attr('data-fullscreen') == 'false' ? false : 769,
    ...
    input.attr('autocomplete', 'off').bind('keyup change', function() {
      ...
      if (term.length > 3 && term != $(this).attr('data-old-term')) {
        ...
        currentAjaxRequest = $.getJSON(searchURL + '&view=json', function(data) {
          console.log('search result why is it empty', data);
          // Reset results.
          resultsList.empty();
          ...
          if(data.results_count == 0) {
            ...
            $.each(data.results, function(index, item) {
              ...
              if(item_title.indexOf("FREE") != -1) return false;
...
var tooltip = {
  ...
  check_pr_page_swatches: function($obj){
    ...
    //console.log('initing', !swc.length && !search.length && !qv.length && !cc.length && !wl.length && !co.length);
...
function showCollectionDesc(){
  let descriptionCont = document.querySelector('.tt-layout-promo-box');
  let showButton = document.getElementById('CD-Show-More');
  let hideButton = document.getElementById('CD-Show-Less');

  descriptionCont.style.maxHeight = "max-content";
  showButton.style.display = "none";
  hideButton.style.display = "block";
}

function hideCollectionDesc(){
  let descriptionCont = document.querySelector('.tt-layout-promo-box');
  let showButton = document.getElementById('CD-Show-More');
  let hideButton = document.getElementById('CD-Show-Less');

  descriptionCont.style.maxHeight = "";
...
document.querySelectorAll('.pdp_accordion-button').forEach(button => {
  button.addEventListener('click', () => {
    button.classList.toggle('pdp_accordion--active');
    $(button.nextElementSibling).slideToggle()
  });
});

function displayNewsletterModal(){
  let modal = document.getElementById('blog_newsletter-modal');
  let pageBody = document.body;

  modal.style.display = "block";
  pageBody.style.scroll = "none";
}
...
function hideNewsletterModal(){
  ...
  modal.style.display = "";
  pageBody.style.scroll = "";
}

function blogAboutReadMore(){
  let aboutTag = document.getElementById('aboutTextMore').parentElement.getElementsByTagName('p');
  let moreButton = document.getElementById('aboutTextMore');
  let lessButton = document.getElementById('aboutTextLess');

  for (let i = 0; i < aboutTag.length; i++) {
    const aboutContent = aboutTag[i];
...
// Toggle blog page sidebar
function showBlogSidebar() {
  document.querySelectorAll('.blogs-toggle_sidebar-mobile').forEach(button => {
    button.classList.add('blog_sidebar--active');
    document.body.style.overflowY = "hidden";
  });
}
function hideBlogSidebar() {
  document.querySelectorAll('.blogs-toggle_sidebar-mobile').forEach(button => {
    button.classList.remove('blog_sidebar--active');
    document.body.style.overflowY = "";
  });
}

// Screen time Select Optional Variant
$('[data-screen-time-option]').on('change', function(){
  const optionals = $(this).parent().find('.tt-title-options');
  const container = $(this);
  const target = $(this).val();
  $.each(optionals, function(i,option){
    if(option.innerHTML.split(':')[0] == container.attr('id')){
      $.each($(option).parent().find('a'), function(j, op){
        if(op.dataset.value == target){
          $(op).click();//select that visually hidden option for me
        }
      });
    }
  })
...
$(document).ready(function(){
  //const hidden_target = $('[data-screen-time-option]').parent().find('.tt-title-options');
  const container = $('[data-screen-time-option]');
  console.log(document.querySelector('.tt-swatches-container-js #boldSelector'));
  if(document.querySelector('[data-screen-time-option]')){
    setTimeout(function(){
      const target_variants = document.querySelector('.tt-swatches-container-js').childNodes;
      target_variants.forEach(function(elm){
        elm.childNodes.forEach(function(e){
          if(e.innerHTML.split(':')[0] == 'Extras (Optional)'){
            elm.style.width = 0;
            elm.style.height = 0;
...
$('.featured_products-flickity').on( 'staticClick.flickity', function( event, pointer, cellElement, cellIndex ) {
...
document.querySelectorAll('.footer_menu-title').forEach(button => {

  button.addEventListener('click', () => {
    if(button.classList.contains("footer_menu--active")){
      button.classList.remove('footer_menu--active');
    }else{
      button.classList.add('footer_menu--active');
    }
...
// Function to update announcement bar height CSS variable
function updateAnnouncementBarHeight() {
  const announcementMessage = document.querySelector('.announcement-bar__message');
  if (announcementMessage) {
    const height = announcementMessage.offsetHeight;
...
function debouncedResize() {
  ...
  resizeTimeout = setTimeout(updateAnnouncementBarHeight, 100);
}
...
window.addEventListener('orientationchange', function() {
  // Small delay to ensure layout is recalculated after orientation change
  setTimeout(updateAnnouncementBarHeight, 200);
});

// Also update when the page becomes visible (handles mobile browser tab switching)
...
window.addEventListener('scroll', function() {
  ...
  const header = document.querySelector('.custom_header-outer');
  if(this.scrollY > announcement.offsetHeight + header.offsetHeight && !announcement.classList.contains('announcement--stuck')) {
    announcement.classList.add('announcement--stuck');
    header.classList.add('header--stuck');
    setTimeout(function() {
      announcement.classList.add('announcement--opening');
      header.classList.add('header--opening');
    }, 100);
  }
  else if(this.scrollY < announcement.offsetHeight + header.offsetHeight && announcement.classList.contains('announcement--stuck')) {
    announcement.classList.remove('announcement--stuck');
    header.classList.remove('header--stuck');
    setTimeout(function() {
      announcement.classList.remove('announcement--opening');
